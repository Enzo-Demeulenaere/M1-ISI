...
def main():
    ...
    command1 = f'echo -n "{file1}" | openssl enc -aes-256-cbc -a -d -k {pswd2} -pbkdf2'
    command2 = f'echo -n "{file2}" | openssl enc -aes-256-cbc -a -d -k {pswd1} -pbkdf2'
    ...
    fileName = "Disque/Cartes_Crypté.txt"
    newFileName = "Disque/Cartes_Decrypté.txt"
    command = f'openssl enc -aes-256-cbc -a -d -k {masterkey} -pbkdf2 -in "{fileName}" -out "{newFileName}"'
